...
from blog.models import Blog, Category, Comment, TagProfile, Image, Visitor, Message, Resource
    ...
    def get(self, request, pk):
        ...
        if article.password != utils.PARAS().BLOG_DEFAULT_PASSWORD:
            if not request.GET.get('psd') or request.GET.get('psd') != article.password:
                return HttpResponse("阿欧~密码不正确咩~")
        # # 渲染markdown
        # article_content = markdown.markdown(article.content, extensions=['markdown.extensions.extra',
